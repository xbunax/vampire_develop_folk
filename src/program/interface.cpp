...
bool match_input_parameter(std::string const key, std::string const word,
  ...
  // Check for valid key, if no match return false - note use of sim: module
  // name for semantic reasons program variables are generally related to
  // simulations but a logically separated for ease of the programmer
  ...
  if (word == test) {
    ...
    if (value == test) {
      ...
      std::cerr << "Error - value for \'sim:" << word
  ...
  if (word == test) {
    ...
    vin::check_for_valid_positive_value(pt, word, line, prefix, unit, "time",
                                        0.0, 1.0, "input", "0.0 - 1s");
  ...
  if (word == test) {
    ...
    // Test for valid range
    vin::check_for_valid_positive_value(rt, word, line, prefix, unit, "time",
                                        0.0, 1.0, "input", "0.0 - 1s");
    // save sanitized value
  ...
  if (word == test) {
    ...
    // Test for valid range
    vin::check_for_valid_positive_value(ft, word, line, prefix, unit, "time",
                                        0.0, 1.0, "input", "0.0 - 1s");
    // save sanitized value
  ...
  if (word == test) {
    ...
    vin::check_for_valid_value(ma, word, line, prefix, unit, "", 0.0, 180.1,
                               "input", "0 - 180 degrees");
  ...
  if (word == test) {
    ...
    vin::check_for_valid_value(ai, word, line, prefix, unit, "", 1.0, 90.0,
                               "input", "1 - 90 degrees");
  ...
  if (word == test) {
    double pt = atof(value.c_str()); // convert string to uint64_t
    // Test for valid range
    vin::check_for_valid_positive_value(pt, word, line, prefix, unit, "none",
                                        -100.0, 100.0, "input", "0.0 - 1");
    // save sanitized value
    program::internal::electrical_strength = pt;
    return true;
  }
  //--------------------------------------------------------------------
  ...
  if (word == test) {
    if (value == "gaussian" or value == "square") {
      ...
      std::cout << "electrical pulse must be gaussian or square" << std::endl;
      return false;
    }
  }
  // Keyword not found
...
bool match_material_parameter(std::string const word, std::string const value,
  ...
  // Check for material id > current array size and if so dynamically expand
  // mp array
  if ((unsigned int)super_index + 1 > internal::mp.size() &&
      super_index + 1 < 101)
